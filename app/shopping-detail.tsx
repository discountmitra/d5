...
const RAW: Array<Omit<Item, 'payPrice'>> = [
  {
    ...
    description: "Pay ₹19 Get 10% Discount Coupon ( Pay 900 and get 1000 Shopping Voucher )",
  ...
  },
  {
    ...
    specialist: "Men, Women & Kids Shopping mall",
    description: "Pay ₹9 Get 5% Discount Coupon ( Pay 450 and get 500 Shopping Voucher )",
  ...
  },
  {
    ...
    name: "Trends",
    specialist: "Store",
    description: "Pay ₹9 Get 5% OFF Min: ₹100.00 Max: ₹10,000.00 Instant Voucher ( Pay 450 and get 500 Shopping Voucher )",
  ...
  },
  {
    ...
    description: "Pay ₹19 & Get 10% Discount Cash Voucher Valid on Online Shopping Purchases: ₹1,000 – ₹50,000 Instant Voucher | Limited Period Offer ( Pay 400 and get 500 Shopping Voucher )",
...
export default function ShoppingDetailScreen() {
  ...
  const getDiscountPercentage = (desc: string): number => {
    const p = desc.match(/(\d+)\%/);
    if (p) return parseInt(p[1], 10);
    return 5;
  };
  const discountPercentage = getDiscountPercentage(item?.description ?? '');
